/**
 * MemoryStore — async drop-in replacement for ChatStore, backed by AgentCore Memory.
 *
 * All methods are async and require an actorId parameter (user identity).
 * STM sessions store conversation events; a dedicated registry session tracks chat metadata.
 */
...
class MemoryStore {
  ...
  /**
   * Ensure a chat exists in the registry (auto-create if needed for multi-instance support).
   */
